...
> **Transform any Agentic IDE into a High-Rigor, Polyglot Scientific Workbench.**
...
Instead of writing ungrounded code or hallucinating figures, the agent is constrained by formal methodological protocols, verifiable provenance, and FAIR data principles, preregistered hypotheses, multi-language pipelines, and cryptographic provenance tracing.
...
1. **Epistemological Guardrails**:
   ...
   - Enforces raw data integrity through strict SHA-256 checksum verification (`01_data/raw/`).
...
| :-------------------------------------- | :-------------------------------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------- |
| **1. Human-in-the-Loop Gates**    | Mitigating HARKing and unrecorded exploratory sweeps                              | Digital signature checkpoints locking hypotheses and models before raw data exposure.                                | `python gate_check.py lock --gate hypothesis-lock` |
| **2. Auto-Appendix for Journals** | Researchers spending days manually writing technical supplementary info.          | Standardized, audit-ready LaTeX/Markdown supplementary materials                                                     | `python generate_appendix.py --format all`         |
...
| **5. Adversarial Reviewer 2**     | Stress-testing claims against confirmation bias and marginal significance         | Hostile red-team auditing: marginal p-value checks, 5% outlier trimming, permutation tests, and multiverse analysis. | `00_meta/adversarial_audit.md`                     |
...
By default, this skill lives inside the local workspace (`.agents/skills/research-scientist/`).To make this skill **globally available across every project** you open in Google Antigravity:
